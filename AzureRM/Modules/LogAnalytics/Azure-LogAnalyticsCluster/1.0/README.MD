...
|------|-------------|------|---------|:--------:|
| <a name="input_identity"></a> [identity](#input\_identity) | (Required) A identity block as defined below. Changing this forces a new Log Analytics Cluster to be created. | <pre>object({<br>    type = string<br>  })</pre> | <pre>{<br>  "type": "SystemAssigned"<br>}</pre> | no |
...
| <a name="input_name"></a> [name](#input\_name) | (Required) The name which should be used for this Log Analytics Cluster. Changing this forces a new Log Analytics Cluster to be created. | `string` | n/a | yes |
...
| <a name="input_size_gb"></a> [size\_gb](#input\_size\_gb) | (Optional) The capacity of the Log Analytics Cluster specified in GB/day. Defaults to 1000. | `number` | `1000` | no |
