...
#<TODO> Add a local data lookup for virtuak hub route table ID & virtual hub connection when enabled by Terraform AzureRM provider
locals {
  ...
  next_hop_id = var.next_hop.id == null ? (
    var.next_hop.name == null && var.next_hop.resource_group_name == null && var.next_hop.virtual_hub_name == null ? (
      var.virtual_hub_connections[var.next_hop.tag].id
