resource "azurerm_lb" "this" {
  ...
  dynamic "frontend_ip_configuration" {
    ...
    content {
      ...
      private_ip_address = frontend_ip_configuration.value.private_ip_address == null && (frontend_ip_configuration.value.
      private_ip_address_allocation == null || frontend_ip_configuration.value.private_ip_address_allocation == "Dynamic") ? null : frontend_ip_configuration.value.private_ip_address

      private_ip_address_allocation = frontend_ip_configuration.value.private_ip_address_allocation == null ? "Dynamic" : frontend_ip_configuration.value.private_ip_address_allocation

      private_ip_address_version = frontend_ip_configuration.value.private_ip_address_version == null ? "IPv4" : frontend_ip_configuration.value.private_ip_address_version
