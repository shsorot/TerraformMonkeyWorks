...
| <a name="input_location"></a> [location](#input\_location) | (Required) The Azure Region where the SSH Public Key should exist. <br>  Changing this forces a new SSH Public Key to be created. | `string` | `null` | no |
| <a name="input_name"></a> [name](#input\_name) | (Required) The name which should be used for this SSH Public Key. <br>  Changing this forces a new SSH Public Key to be created. | `string` | n/a | yes |
| <a name="input_public_key"></a> [public\_key](#input\_public\_key) | (Required) SSH public key used to authenticate to a virtual machine through ssh. <br>    The provided public key needs to be at least 2048-bit and in ssh-rsa format. | `string` | n/a | yes |
...
| <a name="input_tags"></a> [tags](#input\_tags) | (Optional) A mapping of tags which should be assigned to the SSH Public Key. | `map(string)` | `{}` | no |
